...
- 将需要组成batch的entry分类为三种类型的entry，用三个不同的列表来表示，
    ...
    3. waiting entry 这种条目虽然已经被调度，但是仍有一些没有条件没有满足，不可被集成到input_batch，但其运行的显存(page_kv_cache)等资源已经被分配成功，转化为ready_for_forwarding很快（比如在PD分离架构或者cpu_kv_offloading等架构，其page_kv_cache分配已经完毕，但是真实数据还没传输拷贝到对应的page位置）。注：在实验demo中，为了保持简洁性，不会实现这种entry。

Common Data Bus (CDB)

- 该结构负责广播一些消息给各个线程，RS，或者事件，让那些线程能够处理一些事情。
- 广播的消息有：token dependency (解决token依赖问题)，request finished (解决显存释放问题)
- 在实现上，我们不用广播，而是用点对点通信。

## 三个核心component

**Scheduler**：调度器，负责调度request，分配资源，形成entry，放入RS。
...
- 该scheduler会在`Assembler`发射input_batch_i后去立即形成preschedule_entry给RS，然后`Assembler`收到这些entry之后就可以形成prescheduled_input_batch_i+1，然后等待token_dependency解决后变成input_batch。
- 同时，接受CDB发过来的request finished消息，将对应的kv_cache_page标记上free，可以分配给其他request。

**Assembler**：使用事件驱动(Event-driven)机制，只要发现了ready状态的条目，就可以持续集成input_batch。
...
**Executor**：负责执行function（最开始将其设计为整个的model_forwarding函数）
...
4.  4.1 Executor: Do LLM forwarding. 

    4.2 **Meanwhile** Scheduler do preschedule, and then 
    
    4.3 Batch Assembler: forming the `prescheduled_Batch_i+1`. In this way, we can overlap the LLM forwarding and assembling Batch. In general, the `prescheduled_Batch_i+1` will finish before LLM forwarding.
